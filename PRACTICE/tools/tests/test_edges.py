...
def run(root, tool, *args, stdin=None):
    r = subprocess.run([sys.executable, str(root / "PRACTICE" / "tools" / tool), *args], capture_output=True,
                       text=True, encoding="utf-8", env=ENV, input=stdin or "")
...
c, o = run(r, H, "redeem", "--now", "2026-09-19T08:03", "--item", "gas", "--dollars", "0"); check("zero spending refused", c == 1, o)
c, o = run(r, H, "redeem", "--now", "2026-09-19T08:03", "--item", "gas", "--dollars", "-5"); check("negative spending refused", c == 1, o)
c, o = run(r, H, "redeem", "--now", "2026-09-19T08:03", "--item", "gas", "--dollars", "5.125"); check("spending finer than a cent refused", c == 1, o)
...
check("clean bonus paid then clawed back when the second skill misses", "+$4.00" in o1 and "Money: -$2.00" in o2, o1 + " | " + o2)
...
# --- clock edges
r = fresh()
for t, want in (("2026-09-21T03:59", "closed"), ("2026-09-21T07:59", "closed"), ("2026-09-21T08:00", "day window"),
                ("2026-09-21T16:29", "day window"), ("2026-09-21T16:30", "had to start by 16:30"),
                ("2026-09-21T18:15", "closed"), ("2026-09-21T19:00", "evening"), ("2026-09-21T22:59", "evening"),
                ("2026-09-21T23:00", "closed"), ("2026-09-22T00:00", "closed")):
...
run(r, C, "hook-prompt", "--now", "2026-09-21T16:00")
run(r, C, "hook-prompt", "--now", "2026-09-21T16:20")
run(r, C, "hook-prompt", "--now", "2026-09-21T16:40")
c, o = run(r, C, "status", "--now", "2026-09-21T16:45"); check("a day session started before 16:30 keeps running past it", "40 min used" in o, o)
c, o = run(r, C, "status", "--now", "2026-09-21T18:15"); check("18:15 closes the day window mid-session", "closed" in o, o)
(r / "PRACTICE" / "time.csv").write_text("study_day,window,event,time\ngarbage row\n2026-09-21,evening,start,not-a-time\n", encoding="utf-8")
c, o = run(r, C, "hook-prompt", "--now", "2026-09-21T20:00"); check("corrupt time.csv rows are skipped and the hook still answers", c == 0 and "CLOCK:" in o, o)
c, o = run(r, H, "status", "--now", "2026-09-21T20:00"); check("harness status survives a corrupt time.csv", c == 0, o)

# --- two computers, union merge
base = Path(tempfile.mkdtemp())
def git(cwd, *a):
    return subprocess.run(["git", *a], cwd=cwd, capture_output=True, stdin=subprocess.DEVNULL, text=True)
